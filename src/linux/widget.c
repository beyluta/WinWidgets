...
static void
parse_and_get_2d_value(const string html, const size_t html_length, const string tag, size_t *a, size_t *b)
{
        string8_t temp_string;
        if (!ww_begin_parsing(html,
                              html_length,
                              tag,
                              temp_string,
                              sizeof(temp_string)))
...
static bool
parse_and_get_value(const string html, const size_t html_length, const string tag, const parse_type_t type, void *pointer)
{
        string8_t temp_string;
        if (!ww_begin_parsing(html,
                              html_length,
...
void
on_mouse_button_press(void *data, const ww_window_mouse_press_event_t code)
{
        printf("Button pressed is: %hhu\n", code);
        // window_t *self = (window_t *)data;
        // gint x, y;
        // GetMousePosition(&x, &y);
        // printf("Mouse position is %d and %d\n", x, y);
...
static window_t *
window_child_new(window_t *const parent, const string url, const string html, const size_t html_length)
{
        string8_t application_title;
        if (!parse_and_get_value(html, html_length, (string)TAG_APP_NAME, PARSE_TYPE_STRING, application_title))
        {
                return nullptr;
        }
        ...
        parse_and_get_2d_value(html, html_length, (string)TAG_WIN_SIZE, &width, &height);

        size_t x = DEF_X;
        size_t y = DEF_Y;
        parse_and_get_2d_value(html, html_length, (string)TAG_WIN_LOCATION, &x, &y);

        size_t opacity = DEF_OPACITY;
        parse_and_get_value(html, html_length, (string)TAG_WIN_OPACITY, PARSE_TYPE_UINT8, &opacity);

        size_t radius = DEF_RADIUS;
        parse_and_get_value(html, html_length, (string)TAG_WIN_BORD_RAD, PARSE_TYPE_UINT8, &radius);

        bool show_title_bar = DEF_SHOW_TITLE_BAR;
        parse_and_get_value(html, html_length, (string)TAG_SHOW_TITLE_BAR, PARSE_TYPE_BOOLEAN, &show_title_bar);

        bool is_top_most = DEF_TOPMOST;
        parse_and_get_value(html, html_length, (string)TAG_APP_TOPMOST, PARSE_TYPE_BOOLEAN, &is_top_most);

        window_t *child = nullptr;
        if ((child = ww_window_new((window_t){
                     ...
                     .is_child = true,
                     .is_top_most = is_top_most})) == nullptr)
        {
                return nullptr;
        }
...
on_widget_container_clicked(void *, void *webkit_data, void *user_data)
{
        ...
        if ((js_string = webkit_javascript_result_get_js_value(result)) == nullptr)
        {
                return;
        }
        ...
        string_builder_t *temp_file_path = string_builder_slice(file_path, 0, 7);
        ...
        if (ww_get_file_content(temp_file_path->data, html_raw_content, sizeof(html_raw_content)))
        {
                goto cleanup;
        }

        window_t *self = (window_t *)user_data;
        window_t *child = window_child_new(self, file_path->data, html_raw_content, sizeof(html_raw_content));
        if (child == nullptr)
        {
                goto cleanup;
        }
...
on_document_object_model_loaded(void *, void *, void *data)
{
        ...
        for (ww_file_t *current_file = file; current_file != nullptr; current_file = current_file->next)
        {
                string16_t file_content;
                string12_t absolute_file;
                snprintf(absolute_file, MAX_STR_SIZE, "%s/%s", default_dir, current_file->name);
                if (ww_get_file_content(absolute_file, file_content, MAX_FILE_SIZE))
                {
                        fprintf(stderr, "Content of the HTML file could not be loaded into buffer\n");
                        continue;
                }

                string12_t file_title;
                if (!ww_begin_parsing(file_content, sizeof(file_content), TAG_APP_NAME, file_title, sizeof(file_title)))
                {
                        fprintf(stderr, "Failed to get title from HTML content\n");
                        continue;
                }

                string16_t func_args;
                snprintf(func_args, MAX_STR_SIZE * 2, "window.addWidget && window.addWidget(\"%s\", \"%s\")", file_title, absolute_file);
...
main()
{
        ...
        window_t *main_window = ww_window_new((window_t){
                .title = "Main window",
                .width = 1000,
                .height = 1000,
                .x = 0,
        ...
        main_window->vtable->register_event_callback(main_window,
